...
def get_ALLPromoCode():
    response = DataBase.request("SELECT `id`, `name`, `code`, `percent`, `discount`,`limitation_use`,`count` FROM `promo_codes`")
...
def create_promo_code(name, code, percent, discount,limitation_use,count):
    return DataBase.request(
        ...
        {"name": name, "code": code, "percent": percent, "discount": discount,"limitation_use":limitation_use,"count":count})
