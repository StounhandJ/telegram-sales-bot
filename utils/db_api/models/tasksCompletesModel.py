...
def create_task_complete(userID, orderID, departmentTAG, message, document):
    return DataBase.request(
        ...
        {"userID": userID, "orderID": orderID, "departmentTAG": departmentTAG, "message": message, "document": json.dumps(document),
         "date": int(time.time())})
...
def del_task_duplicate(userID, orderID):
    return DataBase.request(
        "DELETE FROM `tasks_completes` WHERE `userID`=%(userID)s AND orderID=%(orderID)s",
